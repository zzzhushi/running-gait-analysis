...
class VideoIngestor:
            ...
            items.append(
                {
                    ...
                    "cached": self.cache.has_any(path.stem),
                }
            )
    ...
    def ingest(self, video_stem: str, view: str, *, force: bool = False) -> IngestedPose:
        ...
        cache_path = self.cache.path_for(video_stem, view)

        with self._lock_for(cache_path):
            if not force and self.cache.exists(video_stem, view):
                try:
            ...
            with self.cache.staging_path(video_stem, view) as staged_path:
                try:
