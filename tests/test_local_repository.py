...
def test_initialize_refuses_legacy_schema_without_losing_runs(tmp_path):
    ...
    with pytest.raises(SchemaError, match="no schema metadata"):
        SQLiteRepository(db_path).initialize()

    with sqlite3.connect(db_path) as conn:
        assert conn.execute("SELECT result_json FROM runs").fetchone()[0] == (
            "{\"legacy\": true}"
        )
        assert conn.execute(
...
def test_initialize_refuses_unknown_schema_without_deleting_its_data(tmp_path):
    ...
    with pytest.raises(SchemaError, match="no data was changed"):
        SQLiteRepository(db_path).initialize()
...
def test_initialize_refuses_a_future_version_without_deleting_its_data(tmp_path):
    ...
    with pytest.raises(UnsupportedSchemaVersion, match="requires version 2"):
        SQLiteRepository(db_path).initialize()
...
def test_concurrent_demo_seed_inserts_each_identity_once(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        added = list(
            pool.map(
                lambda _index: application.seed_demo_runs(),
                range(4),
            )
        )
    ...
    assert [user["name"] for user in repository.list_users()] == ["Demo"]
