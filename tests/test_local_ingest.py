...
def test_view_is_part_of_cache_identity(tmp_path):
    ...
    rear = rear_ingestor.ingest("morning.run", "rear")

    assert rear.cached is False
    assert rear.sequence.view == "rear"
    assert len(rear_runner.calls) == 1
    assert cache.path_for("morning.run", "side-left").is_file()
    assert cache.path_for("morning.run", "rear").is_file()


def test_legacy_cache_is_reused_only_when_its_recorded_view_matches(tmp_path):
    runner = _WritingRunner({**_pose(), "view": "rear"})
    ingestor, cache = _ingestor(tmp_path, runner)
    cache.pose_dir.mkdir()
    cache.legacy_path_for("morning.run").write_text(
        json.dumps(_pose(source="legacy")),
        encoding="utf-8",
    )

    matching = ingestor.ingest("morning.run", "side-left")
    mismatched = ingestor.ingest("morning.run", "rear")

    assert matching.cached is True
    assert matching.sequence.source == "legacy"
    assert mismatched.cached is False
    assert mismatched.sequence.view == "rear"
    assert len(runner.calls) == 1
...
def test_failed_forced_extraction_cannot_overwrite_a_good_cache(tmp_path):
    ...
    ingestor.ingest("morning.run", "side-left")
    original = cache.path_for("morning.run", "side-left").read_bytes()
    ...
    assert cache.path_for("morning.run", "side-left").read_bytes() == original
...
def test_corrupt_cache_is_repaired_only_after_valid_extraction(tmp_path):
    ...
    ingestor, cache = _ingestor(tmp_path, runner)
    cache.pose_dir.mkdir()
    cache.path_for("morning.run", "side-left").write_text(
...
def test_timeout_is_translated_to_a_stable_application_error(tmp_path):
    ...
    assert not cache.exists("morning.run", "side-left")
